...
#### Prisma Schema Usage
In our API call, we interact with database using Prisma. Let's take a look at the API call file in ./biglawai_frontend/pages/api/signin.ts, please pay attention to this line (**const user = await prisma.user.findUnique**) that how we are checking whether user's email exist in the database or not, and if not we should create a new user using his email, name, password in (** ./biglawai_frontend/pages/api/signup.ts**). in that file you will find a function that call prisma.user.create:
